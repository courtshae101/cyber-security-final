...
labs(x="Month of Breach", y="Count of Breaches", title="Facet Bar Graphs of Breaches Distribution in each year")
```
...
```{r}
ggplot(data = cyberdata_edited, mapping = aes(Date_of_Breach)) +  geom_histogram(binwidth = 50, color="black") +  
...
ggplot(data = cyberdata_DB_outliers, mapping = aes(Date_of_Breach)) + geom_histogram(binwidth = 50, fill="red") +  
...
```{r}
ggplot(data = cyberdata_edited) + 
geom_count(mapping = aes(x = month(Date_Posted_or_Updated), y = day(Date_Posted_or_Updated), color = month(Date_Posted_or_Updated))) + scale_x_continuous(breaks=c(1:6))+ 
...
```{r}
ggplot(data = cyberdata_DPoU_outliers, mapping = aes(Date_Posted_or_Updated)) + geom_histogram(binwidth = 10, fill=NA, color="red") + 
...
labs(x="Month of Date Posted or Updated", y="Detection length of Breach", title="Scatterplot of Month Breach was Posted vs How long it took to Post")
```
...
labs(x="Day of Date Posted or Updated", y="Detection length of Breach", title="Boxplot distribution of Detection length of Breach for each Month", subtitle="(Date Posted or Updated)")
```
...
The graph below shows that a majority of the days that have outliers in the Date Posted or Updated variable is around day 17. The median of the boxplot is a little less than the 1000 Detectin length of Breach mark, meaning that most detection lengths are small. 

```{r}
ggplot(data = cyberdata_edited, mapping = aes(x = day(Date_Posted_or_Updated), y = detection_length)) + 
geom_boxplot() + 
labs(x="Day of Date Posted or Updated", y="Detection length of Breach", title="Boxplot of Detection length versus Day", subtitle="Date Posted or Updated")
...
labs(x="Month of Date of Breach", y="Detection length of Breach", title="Scatterplot of Detection length vs Month of Breach", subtitle="Date of Breach")
```
...
```{r}
ggplot(data = cyberdata_edited) + 
geom_boxplot(mapping = aes(x = month_of_breach, y= detection_length)) + labs(x="Month of Breach", y="Detection length of Breach (days)", title="Boxplot distribution of Detection length vs Month of Breach", subtitle="Date of Breach")
```

I reorderd the boxplot of Detection length of Breach versus the year of breach to notice how the median changed throughout the years. 

```{r}
ggplot(data = cyberdata_edited) + 
geom_boxplot(mapping = aes(x = reorder(year,detection_length, FUN = median), y=detection_length)) + 
labs(x="Year of Breach", y="Detection length of Breach", title="Boxplot distribution of Detection length vs Year of Breach", subtitle="Date of Breach")
```
...
```{r}
ggplot(data = cyberdata_edited, mapping = aes(detection_length)) + geom_histogram(binwidth = 50, color="darkgreen") + 
labs(x="Detection Length of Breach (days)", y="Count of Lengths", title="Histogram of Detection Length of Breach", subtitle = "Separated into year frames") + 
...
labs(x="Detection Length of Breach", y="Count of Lengths", title="Polygraph of Detection lengths in States")
```
...
geom_hex(mapping = aes(x = Individuals_Affected, y = detection_length)) + labs(x="Individuals Affected", y="Detection length of Breach (days)", title="Hex graph of Detection lengths vs Individuals affected") 
```
...
geom_point(mapping = aes(x = Individuals_Affected, y = detection_length)) + labs(x="Individuals Affected", y="Detection length of Breach(days)", title="Scatterplot of Detection lengths vs Individuals affected", subtitle="(Concentrated data)")
```
...
ggplot(data = cyberdata_BS_outliers, mapping = aes(breach_start)) + geom_histogram(binwidth = 50, fill=NA, color="red") + 
labs(x="Breach start date", y="Count of date", title="Histogram of Breach start dates", subtitle="(Edited)")
```
...
ggplot(data = cyberdata_BE_outliers, mapping = aes(breach_end)) + geom_histogram(binwidth = 50, fill=NA, color="red") +
...
labs(x="Breach period", y="Count of date", title="Histogram distribution of Breach period", subtitle="(NA replaced)")
```
...
labs(x="Breach period", y="Count of date", title="Histogram distribution of Breach period", subtitle="(Edited)")
```
...
Abline is added that shows that there is no linear trend in the two graphs. For breach period versus detection length, due to there being so many 0 values, the scatterplot shows no trend. The abline added to it doesn't make sense. Hence, this distribution is not modeled.  
...
ggscatter(cyberdata_test_combined, x="Individuals_Affected", y="detection_length", add = "reg.line", cor.coef = TRUE, cor.method = "pearson") + 
