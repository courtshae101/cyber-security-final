...
```{r}
below <- select(filter(breach_data, breach_data$Individuals_Affected < 258121.8), c(Individuals_Affected))

above <- select(filter(breach_data, breach_data$Individuals_Affected > 258121.8), c(Individuals_Affected))
...
breach_data %>% 
  ggplot(aes(x=location_singles, y=below, fill =location_singles)) +geom_col()
